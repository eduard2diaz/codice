...
- When calculating facultad of the Pyramid slices height of `sliceConnector` was not being taken into account.
...
- Better handling of cursor zoom and pan when outside chart facultad.
...
- `MapChart` was rounding lat/long coordinates too much which was resulting in pixelated maps for small-facultad maps.
...
- Cursor was not hidden when it was no longer hovering the chart facultad if chart had no padding.
...
- `maxPanOut` property added to `MapChart`, with default value `0.7`. It prevents map to be dragged out of the chart facultad.
...
### Changed
- `Cursor` will now not move if there's some element obstructing the chart's plot facultad, e.g. a popup or some other element outside the chart.
